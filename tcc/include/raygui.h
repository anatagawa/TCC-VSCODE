...
RAYGUIDEF void GuiLine(Rectangle bounds, float thick);                                                    // Line separator control
...
static unsigned int style[NUM_PROPERTIES] = {
    ...
    0,              // DEFAULT_TEXT_FONT
    12,             // DEFAULT_TEXT_SIZE
    2,              // DEFAULT_TEXT_SPACING                  WARNING!!!!!!!!!!!!! need to update rguilayout!!!!
    1,              // DEFAULT_BORDER_WIDTH
...
static void GuiDrawText(const char *text, int posX, int posY, Color tint)
{
    ...
    DrawTextEx(guiFont, text, (Vector2){ (float)posX, (float)posY }, style[DEFAULT_TEXT_SIZE], style[DEFAULT_TEXT_SPACING], tint);
}
...
// Line control
RAYGUIDEF void GuiLine(Rectangle bounds, float thick)
{
    GuiControlState state = guiState;
...
{
    return GuiImageButtonEx(bounds, texture, (Rectangle){ 0, 0, (float)texture.width, (float)texture.height }, NULL);
}
...
RAYGUIDEF int GuiToggleGroup(Rectangle bounds, const char **text, int count, int active)
{
    ...
    {
        if (i == active) GuiToggleButton((Rectangle){ (float)(bounds.x + i*(bounds.width/count + style[TOGGLEGROUP_PADDING])), (float)(bounds.y), (float)((int)(bounds.width - style[TOGGLEGROUP_PADDING]*(count -1))/count), bounds.height }, text[i], true);
        else if (GuiToggleButton((Rectangle){ (float)(bounds.x + i*(bounds.width/count + style[TOGGLEGROUP_PADDING])), (float)(bounds.y), (float)((int)(bounds.width - style[TOGGLEGROUP_PADDING]*(count -1))/count), bounds.height }, text[i], false) == true) active = i;
    }
...
RAYGUIDEF bool GuiCheckBoxEx(Rectangle bounds, bool checked, const char *text)
{
    ...
    // Draw control
    //--------------------------------------------------------------------
    if (GuiLabelButton((Rectangle){ (float)(bounds.x + bounds.width + GUICHECKBOXEX_WIDTH_PADDING), (float)(bounds.y + bounds.height/2 - style[DEFAULT_TEXT_SIZE]/2 + GUICHECKBOXEX_HEIGHT_PADDING), (float)(textWidth), (float)(style[DEFAULT_TEXT_SIZE]) }, text)) checked = !checked;
...
RAYGUIDEF int GuiComboBox(Rectangle bounds, const char **text, int count, int active)
{
    ...
    Rectangle selector = { bounds.x + bounds.width + style[COMBOBOX_BUTTON_PADDING], 
                               bounds.y, (float)(style[COMBOBOX_SELECTOR_WIDTH]), bounds.height };
...
RAYGUIDEF bool GuiSpinner(Rectangle bounds, int *value, int minValue, int maxValue, int btnWidth, bool editMode)
{
    ...
    Rectangle leftButtonBound = { bounds.x, bounds.y, (float)(btnWidth), bounds.height };
    Rectangle rightButtonBound = { (float)(bounds.x + bounds.width - btnWidth), bounds.y, (float)(btnWidth), bounds.height };
...
RAYGUIDEF bool GuiTextBox(Rectangle bounds, char *text, int textSize, bool editMode)
{
    ...
    if ((state != DISABLED) && !guiLocked)
    {
        ...
        if (editMode)
        {
            ...
            {
                int maxWidth = (bounds.width - (DEFAULT_TEXT_PADDING*2));
                if ((unsigned int)GuiTextWidth(text) < maxWidth - style[DEFAULT_TEXT_SIZE])
                {
                    if ((key >= 32) && (key <= 125))
                    {
                        text[keyCount] = (char)key;
    ...
    if ((state != DISABLED) && !guiLocked)
    {
        ...
        if (editMode)
        {
            ...
            if (keyCount < (textSize - 1))
            {
                ...
                else if ((unsigned int)GuiTextWidth(strrchr(text, '\n')) < (maxWidth - style[DEFAULT_TEXT_SIZE]))
                {                    
                    if ((key >= 32) && (key <= 125))
                    {
                        text[keyCount] = (char)key;
...
RAYGUIDEF float GuiSlider(Rectangle bounds, float value, float minValue, float maxValue)
{
    ...
    Rectangle slider = { bounds.x + (int)((value/(maxValue - minValue))*(bounds.width - 2*style[SLIDER_BORDER_WIDTH])) - style[SLIDER_SLIDER_WIDTH]/2,
                         bounds.y + style[SLIDER_BORDER_WIDTH], (float)style[SLIDER_SLIDER_WIDTH], bounds.height - 2*style[SLIDER_BORDER_WIDTH] };
...
RAYGUIDEF float GuiSliderBar(Rectangle bounds, float value, float minValue, float maxValue)
{
    ...
    Rectangle slider = { bounds.x + style[SLIDERBAR_BORDER_WIDTH], bounds.y + style[SLIDERBAR_BORDER_WIDTH] + style[SLIDERBAR_INNER_PADDING], 
                         (float)((int)(((value - minValue)/(maxValue - minValue))*(bounds.width - 2*style[SLIDERBAR_BORDER_WIDTH]))),
...
RAYGUIDEF float GuiProgressBarEx(Rectangle bounds, float value, float minValue, float maxValue, bool showValue)
{
    ...
    // Draw control
    //--------------------------------------------------------------------
    if (showValue) GuiLabel((Rectangle){ bounds.x + bounds.width + PROGRESSBAREX_WIDTH_PADDING, bounds.y + bounds.height/2 - style[DEFAULT_TEXT_SIZE]/2 + PROGRESSBAREX_HEIGHT_PADDING, (float)style[DEFAULT_TEXT_SIZE], (float)style[DEFAULT_TEXT_SIZE] }, FormatText("%.02f", value));
    ...
    switch (state)
    {
        case NORMAL:
        {
            ...
            for (int i = startIndex; i < endIndex; i++)
            {
                ...
                {
                    GuiDisable();
                    GuiListElement((Rectangle){ (float)posX, bounds.y + style[LISTVIEW_ELEMENTS_PADDING] + LISTVIEW_LINE_THICK + (i - startIndex)*(style[LISTVIEW_ELEMENTS_HEIGHT] + style[LISTVIEW_ELEMENTS_PADDING]), (float)elementWidth, (float)style[LISTVIEW_ELEMENTS_HEIGHT] }, text[i], false, false);
                    GuiEnable();
                }
                else if (i == auxActive) GuiListElement((Rectangle){ (float)posX, bounds.y + style[LISTVIEW_ELEMENTS_PADDING] + LISTVIEW_LINE_THICK + (i - startIndex)*(style[LISTVIEW_ELEMENTS_HEIGHT] + style[LISTVIEW_ELEMENTS_PADDING]), (float)elementWidth, (float)style[LISTVIEW_ELEMENTS_HEIGHT] }, text[i], true, false);
                else GuiListElement((Rectangle){ (float)posX, bounds.y  + style[LISTVIEW_ELEMENTS_PADDING] + LISTVIEW_LINE_THICK + (i - startIndex)*(style[LISTVIEW_ELEMENTS_HEIGHT] + style[LISTVIEW_ELEMENTS_PADDING]), (float)elementWidth, (float)style[LISTVIEW_ELEMENTS_HEIGHT] }, text[i], false, false);
            }
        } break;
        case FOCUSED:
        {
            ...
            for (int i = startIndex; i < endIndex; i++)
            {
                ...
                {
                    GuiDisable();
                    GuiListElement((Rectangle){ (float)posX, bounds.y + (float)style[LISTVIEW_ELEMENTS_PADDING] + LISTVIEW_LINE_THICK + (i - startIndex)*(style[LISTVIEW_ELEMENTS_HEIGHT] + style[LISTVIEW_ELEMENTS_PADDING]), (float)elementWidth, (float)style[LISTVIEW_ELEMENTS_HEIGHT] }, text[i], false, false);
                    GuiEnable();
                }
                else if (i == auxActive) GuiListElement((Rectangle){ (float)posX, bounds.y + style[LISTVIEW_ELEMENTS_PADDING] + LISTVIEW_LINE_THICK + (i - startIndex)*(style[LISTVIEW_ELEMENTS_HEIGHT] + style[LISTVIEW_ELEMENTS_PADDING]), (float)elementWidth, (float)style[LISTVIEW_ELEMENTS_HEIGHT] }, text[i], true, false);
                else GuiListElement((Rectangle){ (float)posX, bounds.y  + style[LISTVIEW_ELEMENTS_PADDING] + LISTVIEW_LINE_THICK + (i - startIndex)*(style[LISTVIEW_ELEMENTS_HEIGHT] + style[LISTVIEW_ELEMENTS_PADDING]), (float)elementWidth, (float)style[LISTVIEW_ELEMENTS_HEIGHT] }, text[i], false, false);
        ...
        case PRESSED:
        {
            ...
            for (int i = startIndex; i < endIndex; i++)
            {
                ...
                {
                    if (GuiListElement((Rectangle){ (float)posX, bounds.y + style[LISTVIEW_ELEMENTS_PADDING] + LISTVIEW_LINE_THICK + (i - startIndex)*(style[LISTVIEW_ELEMENTS_HEIGHT] + style[LISTVIEW_ELEMENTS_PADDING]), (float)elementWidth, (float)style[LISTVIEW_ELEMENTS_HEIGHT] }, text[i], true, true) == false) auxActive = -1;
                }
                else
                {
                    if (GuiListElement((Rectangle){ (float)posX, bounds.y  + style[LISTVIEW_ELEMENTS_PADDING] + LISTVIEW_LINE_THICK + (i - startIndex)*(style[LISTVIEW_ELEMENTS_HEIGHT] + style[LISTVIEW_ELEMENTS_PADDING]), (float)elementWidth, (float)style[LISTVIEW_ELEMENTS_HEIGHT] }, text[i], false, true) == true) auxActive = i;
                }
            }
        } break;
        case DISABLED:
        {
            ...
            for (int i = startIndex; i < endIndex; i++)
            {
                if (i == auxActive) GuiListElement((Rectangle){ (float)posX, bounds.y + style[LISTVIEW_ELEMENTS_PADDING] + LISTVIEW_LINE_THICK + (i - startIndex)*(style[LISTVIEW_ELEMENTS_HEIGHT] + style[LISTVIEW_ELEMENTS_PADDING]), (float)elementWidth, (float)style[LISTVIEW_ELEMENTS_HEIGHT] }, text[i], true, false);
                else GuiListElement((Rectangle){ (float)posX, bounds.y  + style[LISTVIEW_ELEMENTS_PADDING] + LISTVIEW_LINE_THICK + (i - startIndex)*(style[LISTVIEW_ELEMENTS_HEIGHT] + style[LISTVIEW_ELEMENTS_PADDING]), (float)elementWidth, (float)style[LISTVIEW_ELEMENTS_HEIGHT] }, text[i], false, false);
            }
        } break;
        default: break;
...
{    
    ...
    Rectangle boundsHue = { bounds.x + bounds.width + style[COLORPICKER_BARS_PADDING], bounds.y, (float)style[COLORPICKER_BARS_THICK], bounds.height };
